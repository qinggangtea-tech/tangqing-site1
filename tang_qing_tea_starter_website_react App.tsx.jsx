...
const LOGO = "sandbox:/mnt/data/LoGo.jpg";
const HERO_BG = "sandbox:/mnt/data/首页.jpg";
...
const Copy = {
  ...
  zh: {
    brand: "唐青 TangQing",
    nav: { about: "什么是小青柑", origin: "原产地", gallery: "图片集", process: "制作工艺", brew: "冲泡方法", storage: "保存与口感", faq: "常见问题", contact: "联系我们" },
    heroTitle: "一颗小青柑，茶里有天地",
    heroSub: "新会青柑与云南普洱的交融，甘香圆润，雅致耐品。",
    aboutTitle: "什么是小青柑 What is Xiao Qing Gan",
    aboutP1: "小青柑= 青柑果皮 + 普洱茶。先将新会产区的青柑晒制/烘制，再装入普洱熟茶，形成独特的柑香与陈香。",
    aboutP2: "Xiao Qing Gan is young green tangerine peel from Xinhui (Guangdong) filled with Pu'er tea from Yunnan, combining citrus aroma with mellow, smooth body.",
    originTitle: "原产地与风土 Origin & Terroir",
    originP1: "青柑：广东新会（柑皮油胞丰富、香气高扬）。普洱：云南（发酵与陈化带来顺滑口感）。",
    originP2: "Peel from Xinhui, Guangdong; Pu'er from Yunnan.",
    processTitle: "制作工艺 Process",
    processSteps: [
      "采摘青柑 · Harvest green tangerines",
      "清洗与打孔 · Clean & pierce",
      "填入普洱 · Fill with Pu'er",
      "低温干燥/晒制 · Dry or sun‑cure",
      "回仓养味 · Rest to balance aroma",
      "分级与包装 · Grade & pack",
    ],
    brewTitle: "冲泡方法 Brewing Guide",
    brewPoints: [
      "器具：盖碗/公道杯。Gaiwan or small pot (150–200ml)",
      "投茶：整颗或半颗。Use whole or half fruit",
      "水温：95–100°C。Water near boiling",
      "醒茶：快速润洗 1 次。Quick rinse once",
      "注水：首泡 15–30 秒，之后按口感加减。1st 15–30s, then adjust",
      "可续泡 6–10 次。6–10 infusions",
    ],
    storageTitle: "保存与口感 Storage & Taste",
    storageP1: "阴凉干燥、避免异味；随时间缓慢转化，柑香更柔、汤感更顺。",
    storageP2: "Store cool & dry; aging softens citrus and deepens sweetness.",
    faqTitle: "常见问题 FAQ",
    FAQ: [
      { q: "口味会很酸吗？", a: "不会。是柑皮的清香与甜润，配合普洱的顺滑。" },
      { q: "能不能当天喝完？", a: "可以。也可装冰箱密封短存 1–2 天再续泡。" },
      { q: "适合送礼吗？", a: "非常适合。体面、好喝、易上手。支持礼盒定制。" },
    ],
    contactTitle: "Contact 合作与联系",
    contactSub: "批发/零售/企业礼赠请邮件或电话。",
    email: "qinggangtea@gmail.com",
    phone: "+1 917-615-6330",
    footer: "© " + new Date().getFullYear() + " 唐青 TangQing. 保留所有权利",
  },
};
...
function GallerySection({ onSetHero, onAddToStory }) {
  const [images, setImages] = useState([
    "sandbox:/mnt/data/首页.jpg",
    "sandbox:/mnt/data/LoGo.jpg",
    "sandbox:/mnt/data/好图.jpg",
    "sandbox:/mnt/data/好图2.jpg",
    "sandbox:/mnt/data/好图3.jpg",
    "sandbox:/mnt/data/好图4.png",
    "sandbox:/mnt/data/好图%201.jpg",
  ]);
...
export default function TangQingSite() {
  ...
  const [aboutImg, setAboutImg] = useState("sandbox:/mnt/data/好图.jpg");
  return (
    <div style={{ background: THEME.paper, fontFamily: "'Noto Serif SC','Songti SC','SimSun','Times New Roman',serif", backgroundImage: "radial-gradient(rgba(0,0,0,0.03) 1px, transparent 1px), linear-gradient(0deg, rgba(200,169,106,0.05), rgba(200,169,106,0.05))", backgroundSize: "10px 10px, auto", backgroundAttachment: "fixed", backgroundBlendMode: "multiply" }} className="min-h-screen text-zinc-900">
            ...
            {storyImages.length > 0 && (
              <div className="grid grid-cols-2 gap-3">
                {storyImages.map((src, i) => (
                  <img key={i} src={src} alt={`story-${i+1}`} className="w-full h-32 object-cover rounded-xl border" style={{borderColor: THEME.accent}} />
                ))}
              </div>
            )}
      ...
      <Section id="origin" title={L.originTitle}>
        <p>{L.originP1}</p>
        <p className="mt-2 italic text-zinc-600">{L.originP2}</p>
        <div className="mt-6 grid sm:grid-cols-2 gap-5">
          <Card>
            <div className="p-4">
              <div className="flex items-center gap-2"><MapPin size={16} style={{ color: THEME.accent }} /><strong>Xinhui 新会</strong></div>
              <p className="mt-1 text-sm">油胞丰富、香气清扬；皮更紧致，转化更稳。</p>
            </div>
          </Card>
          <Card>
            <div className="p-4">
              <div className="flex items-center gap-2"><MapPin size={16} style={{ color: THEME.accent }} /><strong>Yunnan 云南</strong></div>
              <p className="mt-1 text-sm">普洱发酵带来甜润与厚度，耐泡性好。</p>
            </div>
          </Card>
        </div>
      </Section>
      ...
      <CloudDivider />
      <GallerySection onSetHero={(src)=>setHeroBg(src)} onAddToStory={(src)=>setStoryImages((prev)=>[...prev, src])} />

      <CloudDivider />
      <Section id="process" title={L.processTitle}>
      ...
      <CloudDivider />
      <Section id="brew" title={L.brewTitle}>
        <ul className="list-disc pl-5 space-y-2">
          {L.brewPoints.map((s, i) => (
            <li key={i}>{s}</li>
          ))}
        </ul>
      </Section>
      ...
      <CloudDivider />
      <Section id="storage" title={L.storageTitle}>
        <p>{L.storageP1}</p>
        <p className="mt-2 italic text-zinc-600">{L.storageP2}</p>
      </Section>

      <CloudDivider />
      <Section id="faq" title={L.faqTitle}>
      ...
      <Section id="contact" title={L.contactTitle}>
        ...
        <div className="mt-4 grid sm:grid-cols-3 gap-4">
          <a href={`mailto:${L.email}`} className="rounded-xl border px-4 py-3 flex items-center gap-2 text-sm" style={{ borderColor: THEME.primaryDark, color: THEME.primaryDark }}>
        ...
        <form
          ...
          className="mt-6 rounded-2xl border p-5 bg-white/60"
        >
